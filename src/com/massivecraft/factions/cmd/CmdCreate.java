...
public class CmdCreate extends FCommand
{
	...
	public void perform()
	{
		...
		if (fme.hasFaction())
		{
			msg("<b>Vous devez quitter votre faction pour en créer une autre.");
			return;
		}
		
		if (Factions.i.isTagTaken(tag))
		{
			msg("<b>Le nom est déjà utilisé.");
			return;
		}
	...
	// finish setting up the FPlayer
		...
		{
			follower.msg("%s<i> avez crée une nouvelle faction %s", fme.describeTo(follower, true), faction.getTag(follower));
		}
		
		msg("<i>Vous pouvez maintenant : %s", p.cmdBase.cmdDescription.getUseageTemplate());

		if (Conf.logFactionCreate)
			P.p.log(fme.getName()+" à créer une nouvelle faction qui s'intitule : "+tag);
	}
