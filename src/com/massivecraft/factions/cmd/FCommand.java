...
public abstract class FCommand extends MCommand<P>
{
	...
	public boolean validSenderType(CommandSender sender, boolean informSenderIfNot)
	{
		...
		{
			sender.sendMessage(p.txt.parse("<b>Vous êtes membre d'aucune faction."));
			return false;
		}
		
		if (this.senderMustBeOfficer && ! fplayer.getRole().isAtLeast(Rel.OFFICER))
		{
			sender.sendMessage(p.txt.parse("<b>Seul les modérateurs peuvent faire ça %s.", this.getHelpShort()));
			return false;
		}
		
		if (this.senderMustBeLeader && ! fplayer.getRole().isAtLeast(Rel.LEADER))
		{
			sender.sendMessage(p.txt.parse("<b>Seul le chef peut faire ça %s.", this.getHelpShort()));
			return false;
		}
	...
	public boolean assertHasFaction()
	{
		...
		if ( ! fme.hasFaction())
		{
			sendMessage("Vous êtes membre d'aucune faction.");
			return false;
		}
		return true;
	}
	...
	public FPlayer strAsFPlayer(String name, FPlayer def, boolean msg)
	{
		...
		if (msg && ret == null)
		{
			this.msg("<b>Aucun joueur \"<p>%s<b>\" n'a été trouvé.", name);
		}
	...
	public Faction strAsFaction(String name, Faction def, boolean msg)
	{
		...
		if (msg && ret == null)
		{
			this.msg("<b>La faction ou le joueur \"<p>%s<b>\" n'a été trouvé.", name);
		}
	...
	public boolean canIAdministerYou(FPlayer i, FPlayer you)
	{
		...
		{
			i.sendMessage(p.txt.parse("<b>Seulement le chef peut faire ça."));
		}
		...
		{
			i.sendMessage(p.txt.parse("<b>Vous devez être modérateur pour faire ça."));
		}
