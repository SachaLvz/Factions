...
public class CmdJoin extends FCommand
{
	...
	public void perform()
	{
		...
		{
			msg("<b>%s %s est déjà un membre de %s", fplayer.describeTo(fme, true), (samePlayer ? "are" : "is"), faction.getTag(fme));
			return;
		}

		if (Conf.factionMemberLimit > 0 && faction.getFPlayers().size() >= Conf.factionMemberLimit)
		{
			msg(" <b>!<white> Le faction %s est à sa limite de %d membres.", faction.getTag(fme), Conf.factionMemberLimit, fplayer.describeTo(fme, false));
			return;
		}

		if (fplayer.hasFaction())
		{
			msg("<b>%s devez quitter d'abord votre faction .", fplayer.describeTo(fme, true), (samePlayer ? "votre" : "leur"));
			return;
		}
		...
		{
			msg("<i>Pour rejoindre il te faut une invitation jeune homme.");
			if (samePlayer)
				faction.msg("%s<i> essaye de rejoindre votre faction.", fplayer.describeTo(faction, true));
			return;
		}
		...
		faction.msg("<i>%s a rejoint votre faction.", fplayer.describeTo(faction, true));
		fme.msg("<i>%s rejoint avec succès  %s.", fplayer.describeTo(fme, true), faction.getTag(fme));
