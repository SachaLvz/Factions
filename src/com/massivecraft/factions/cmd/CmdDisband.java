...
public class CmdDisband extends FCommand
{
	...
	public void perform()
	{
		...
		{
			msg("<i>C'est une faction permanante vous ne pouvez pas la dissoudre.");
			return;
		}
		...
		for (FPlayer fplayer : FPlayers.i.getOnline())
		{
			...
			{
				fplayer.msg("<h>%s<i> à dissous votre faction.", who);
			}
			else
			{
				fplayer.msg("<h>%s<i> à dissous la faction %s.", who, faction.getTag(fplayer));
			}
		}
		if (Conf.logFactionDisband)
			P.p.log("La faction "+faction.getTag()+" ("+faction.getId()+") à été dissous par "+(senderIsConsole ? "console command" : fme.getName())+".");
